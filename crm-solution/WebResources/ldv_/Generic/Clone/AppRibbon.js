...
Ldv.CloneButton_PopulateAppDropdownMenu = function(commandProperties, entityName, isView)
{
    try
    {
        ...
        var menu =
            '<Menu Id="ldv.ApplicationRibbon.{!EntityLogicalName}.Clone.' + (isView ? 'View' : 'Entity')
                + '.Button.Menu">'
                + '  <MenuSection Id="ldv.ApplicationRibbon.{!EntityLogicalName}.Clone.' + (isView ? 'View' : 'Entity')
                + 'Menu.Section" Sequence="5" DisplayMode="Menu16">'
                + '    <Controls Id="ldv.ApplicationRibbon.{!EntityLogicalName}.Clone.' + (isView ? 'View' : 'Entity')
        ...
        if (!result.responseJSON)
        {
            ...
            {
                console.error('CloneButton_PopulateAppDropdownMenu > No response!');
                return;
            }
        }
        ...
        for (var i = 0; i < result.length; i++)
        {
            ...
            menu +=
                '      <Button Command="ldv.ApplicationRibbon.Clone.' + (isView ? 'View' : 'Entity')
                + 'PopButton.Command" '
                + '        Id="' + id + '" '
                + '        LabelText="' + name + '" '
                + '        Sequence="' + ((i + 1) * 10) + '" />';
...
Ldv.CloneRetrieveDefaultTemplate = function(primaryEntityTypeName)
{
    try
    {
        ...
        if (!result.responseJSON)
        {
            ...
            {
                console.error('CloneRetrieveDefaultTemplate > No response!');
                return;
            }
        }
    ...
    {
        console.error('CloneRetrieveDefaultTemplate');
        console.error(e);
    }
};
